...
import { useSportFlip } from "./useSportFlip";
...
export default function CinematicHome() {
  ...
  const { displayed: cardSport, phase: flipPhase } = useSportFlip(sport);
  const selectedSportRecord = sportRecords.find(s => s.title === cardSport);
  const [sportMotionPaused, setSportMotionPaused] = useState(false);
  const [sportOnScreen, setSportOnScreen] = useState(false);
  const sportsSection = useRef<HTMLElement>(null);
  useEffect(() => {
    const element = sportsSection.current;
    if (!element || !("IntersectionObserver" in window)) { setSportOnScreen(true); return; }
    const observer = new IntersectionObserver(entries => setSportOnScreen(entries[0].isIntersecting), {threshold: 0.05});
  ...
  const sportAthlete=athletes.find(a=>sportKey(a.sport)===sportKey(cardSport)&&a.imageUrl);
  const sportEvents=feeds.events.filter(e=>sportKey(e.sport)===sportKey(cardSport));
  ...
  return <main ref={root} className="cinematic-home">
    <section className="ch-hero" aria-labelledby="ch-title">
      ...
      <div className="ch-hero-copy"><p className="ch-kicker">ONE STATE. LIMITLESS POTENTIAL.</p><h1 id="ch-title"><span className="ch-title-shine">WHERE<br/>MAHARASHTRA</span><br/><TypingText text="MEETS EXCELLENCE." delay={1900} /></h1><div className="ch-hero-support"><div><p className="ch-intro-copy">The ambition of a state.<br/>The spirit of every athlete.</p><Link className="ch-action" to="/events">Enter the action <ArrowUpRight/></Link></div><HeroLeadership/></div></div>
      <div className="ch-hero-bottom"><a href="#ch-sports">SCROLL TO EXPLORE <ArrowDown size={16}/></a><span>01 / THE STARTING LINE</span></div>
    </section>
    <section ref={sportsSection} className="ch-chapter ch-sports" id="ch-sports"><div className="ch-heading"><p className="ch-kicker">02 / FIND YOUR DISCIPLINE</p><h2><span className="ch-title-shine">SPORTS</span><br/><TypingText text="IN MOTION." /></h2></div>{state('sports')}<div className="ch-sport-stage"><div className="ch-sport-browser"><label htmlFor="ch-sport-search">Find your sport</label><input id="ch-sport-search" type="search" placeholder="Search all sports…" value={sportQuery} onChange={e=>setSportQuery(e.target.value)}/><p className="ch-sport-count" role="status">{visibleSports.length} of {sports.length} sports</p><div className="ch-sport-menu" aria-label="Select a sport">{visibleSports.map(s=><button key={s} aria-pressed={s===sport} onClick={()=>setSport(s)}><small>{String(sports.indexOf(s)+1).padStart(2,'0')}</small><span>{s}</span><ArrowUpRight size={20}/></button>)}</div>{sports.length>0&&!visibleSports.length&&<p>No matching sports. Try another name.</p>}</div>{sport&&<div className={"ch-sport-visual ch-flip-" + flipPhase} data-ambient={sportOnScreen && !sportMotionPaused ? "running" : "paused"} aria-busy={flipPhase !== "idle"}><button type="button" className="ch-sport-motion-toggle" aria-pressed={sportMotionPaused} onClick={()=>setSportMotionPaused(value=>!value)}>{sportMotionPaused ? "Resume motion" : "Pause motion"}</button><span className="ch-sport-shimmer" aria-hidden="true"/><span className="ch-sport-edge" aria-hidden="true"/>{selectedSportRecord?.imageUrl?<img src={selectedSportRecord.imageUrl} alt={cardSport} loading="lazy"/>:sportAthlete?<img src={sportAthlete.imageUrl} alt={sportAthlete.title} loading="lazy"/>:<div className="ch-track-art" aria-hidden="true"><i/><i/><i/><i/><i/></div>}<div className="ch-sport-caption"><span>FOCUS / {cardSport.toUpperCase()}</span><h3>{cardSport}</h3><p>{pending?'Loading events…':errors.includes('events')?'Event updates currently unavailable.':`${sportEvents.length} published events`}</p><Link to="/events">Explore the event calendar <ArrowUpRight size={18}/></Link></div></div>}</div></section>
    <section className="ch-chapter ch-events"><div className="ch-section-top"><div><p className="ch-kicker">03 / ON THE CALENDAR</p><h2><span className="ch-title-shine">THE NEXT</span><br/><TypingText text="BIG MOMENT." /></h2></div><Link to="/events">All events <ArrowUpRight/></Link></div>{state('events',events)}<div className="ch-event-list">{events.slice(0,3).map((e,i)=><Link className="ch-event" to={'/events/'+e._id} key={e._id}><span className="ch-event-index">0{i+1}</span><div><span className="ch-kicker">{e.eventStatus} / {e.sport||'SPORT'}</span><h3>{e.title}</h3><p>{[e.venue,e.city].filter(Boolean).join(' · ')||'Venue to be announced'}</p></div><div className="ch-event-date">{e.startDate||'Date to be announced'}<ArrowUpRight/></div></Link>)}</div></section>
    <section className="ch-chapter ch-champions"><p className="ch-kicker">04 / THE PEOPLE BEHIND THE PERFORMANCE</p><h2><span className="ch-title-shine">THE</span> <TypingText text="CHAMPIONS." /></h2>{state('athletes')}{athlete?<div className="ch-athlete" key={athlete._id}><div className="ch-athlete-photo">{athlete.imageUrl?<img src={athlete.imageUrl} alt={athlete.title} loading="lazy"/>:<div className="ch-monogram" aria-hidden="true">MOA</div>}</div><div className="ch-athlete-story"><p className="ch-kicker">{athlete.sport} · {athlete.district||'MAHARASHTRA'}</p><h3>{athlete.title}</h3><p>{athlete.achievements||athlete.description||'Discover this athlete’s journey.'}</p><Link className="ch-action" to={'/athletes/'+athlete._id}>Discover the athlete <ArrowUpRight/></Link></div></div>:<div className="ch-champion-empty"><span>EVERY JOURNEY<br/>STARTS WITH <TypingText text="BELIEF." /></span><Link to="/athletes">Explore our athletes <ArrowUpRight/></Link></div>}{athletes.length>1&&<div className="ch-controls"><button aria-label="Previous athlete" onClick={()=>setChampion((champion+athletes.length-1)%athletes.length)}><ChevronLeft/></button><span>{champion%athletes.length+1} / {athletes.length}</span><button aria-label="Next athlete" onClick={()=>setChampion((champion+1)%athletes.length)}><ChevronRight/></button></div>}</section>
    <section className="ch-chapter ch-medals"><div><p className="ch-kicker">05 / EVERY FINISH TELLS A STORY</p><h2><span className="ch-title-shine">PRIDE.<br/>IN EVERY</span><br/><TypingText text="PERFORMANCE." /></h2><Link to="/medals">Explore the medal tally <ArrowUpRight/></Link></div><div className="ch-medal-board">{['Gold','Silver','Bronze'].map((m,i)=><div className={'ch-medal-row ch-metal-'+i} key={m}><div className="ch-medal" aria-hidden="true"><span>{i+1}</span></div><span>{m.toUpperCase()}</span><strong>{pending||errors.includes('results')?'—':<NumberReveal value={feeds.results.filter(r=>r.medal===m).length}/>}</strong></div>)}<p>Medals from published results</p></div></section>
    <section className="ch-chapter ch-regions"><p className="ch-kicker">06 / CONNECTED BY SPORT</p><h2><span className="ch-title-shine">ACROSS</span><br/><TypingText text="MAHARASHTRA." /></h2><p>Explore the sporting calendar by city and district.</p><div className="ch-region-controls" aria-label="Choose a region">{regions.map(r=><button key={r} aria-pressed={r===region} onClick={()=>setRegion(r)}>{r}</button>)}</div><div className="ch-region-result" aria-live="polite"><h3>{region}</h3>{state('events',localEvents)}{localEvents.slice(0,3).map(e=><Link key={e._id} to={'/events/'+e._id}>{e.title}<ArrowUpRight size={18}/></Link>)}</div></section>
    <section className="ch-chapter ch-results"><div className="ch-section-top"><div><p className="ch-kicker">07 / THE FINISH LINE</p><h2><span className="ch-title-shine">RESULTS THAT</span><br/><TypingText text="SPEAK VOLUMES." /></h2></div><Link to="/results">All results <ArrowUpRight/></Link></div>{state('results')}{[...feeds.results].sort((a,b)=>(b.date||'').localeCompare(a.date||'')).slice(0,4).map(r=><Link className="ch-result-row" to={'/results/'+r._id} key={r._id}><strong>{String(r.position).padStart(2,'0')}</strong><div><h3>{r.athlete}</h3><p>{r.tournament} · {r.sport}</p></div><span>{r.medal==='None'?'RESULT':r.medal.toUpperCase()}</span><ArrowUpRight/></Link>)}</section>
    <section className="ch-chapter ch-news"><div className="ch-section-top"><div><p className="ch-kicker">08 / FROM THE ASSOCIATION</p><h2><span className="ch-title-shine">BEYOND</span><br/><TypingText text="THE ARENA." /></h2></div><Link to="/news">All news <ArrowUpRight/></Link></div>{state('news')}<div className="ch-news-grid">{[...feeds.news].sort((a,b)=>(b.publishDate||'').localeCompare(a.publishDate||'')).slice(0,3).map(n=><Link to={'/news/'+n._id} key={n._id}>{n.imageUrl&&<img src={n.imageUrl} alt="" loading="lazy"/>}<p className="ch-kicker">{n.category||'NEWS'} · {n.publishDate}</p><h3>{n.title}</h3><ArrowUpRight/></Link>)}</div><Link className="ch-document-link" to="/circulars">Official bulletins & circulars <ArrowUpRight/></Link></section>
    <section className="ch-chapter ch-closing"><p className="ch-kicker">ONE MOVEMENT. ONE MAHARASHTRA.</p><h2><span className="ch-title-shine">THE FUTURE<br/>OF SPORT.</span><br/><TypingText text="TOGETHER." /></h2><Link className="ch-action" to="/about/vision">Our vision <ArrowUpRight/></Link></section>
  </main>;
}
