...
export default function CinematicHome() {
  ...
  return <main ref={root} className="cinematic-home">
    ...
    <section className="ch-chapter ch-sports" id="ch-sports"><div className="ch-heading"><p className="ch-kicker">02 / FIND YOUR DISCIPLINE</p><h2><span className="ch-title-shine">SPORTS</span><br/><TypingText text="IN MOTION." /></h2></div>{state('sports')}<div className="ch-sport-stage"><div className="ch-sport-browser"><label htmlFor="ch-sport-search">Find your sport</label><input id="ch-sport-search" type="search" placeholder="Search all sports…" value={sportQuery} onChange={e=>setSportQuery(e.target.value)}/><p className="ch-sport-count" role="status">{visibleSports.length} of {sports.length} sports</p><div className="ch-sport-menu" aria-label="Select a sport">{visibleSports.map(s=><button key={s} aria-pressed={s===sport} onClick={()=>setSport(s)}><small>{String(sports.indexOf(s)+1).padStart(2,'0')}</small><span>{s}</span><ArrowUpRight size={20}/></button>)}</div>{sports.length>0&&!visibleSports.length&&<p>No matching sports. Try another name.</p>}</div>{sport&&<div className={"ch-sport-visual ch-flip-" + flipPhase} aria-busy={flipPhase !== "idle"}>{selectedSportRecord?.imageUrl?<img src={selectedSportRecord.imageUrl} alt={cardSport} loading="lazy"/>:sportAthlete?<img src={sportAthlete.imageUrl} alt={sportAthlete.title} loading="lazy"/>:<div className="ch-track-art" aria-hidden="true"><i/><i/><i/><i/><i/></div>}<div className="ch-sport-caption"><span>FOCUS / {cardSport.toUpperCase()}</span><h3>{cardSport}</h3><p>{pending?'Loading events…':errors.includes('events')?'Event updates currently unavailable.':`${sportEvents.length} published events`}</p><Link to="/events">Explore the event calendar <ArrowUpRight size={18}/></Link></div></div>}</div></section>
